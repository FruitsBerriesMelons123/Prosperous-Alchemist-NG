...
## CACO Physician override

CACO overrides Skyrim's vanilla `Physician` perk record. Its active description is **"Health regenerates 15% faster."**; it does not retain vanilla Physician's 25% multiplier for Restore Health, Restore Magicka, and Restore Stamina potion effects. Therefore, CACO-enabled potion-value predictions must not apply the vanilla Physician potion multiplier. Prosperous Alchemist preserves the vanilla 1.25 Physician behavior only when CACO is inactive, while the CACO path follows the overridden perk behavior.
...
The adapter initializes at SKSE's `kDataLoaded` message. It first identifies a loaded full or light CACO plugin (`Complete Alchemy & Cooking Overhaul.esp` or `.esm`) and then resolves the core lists, six duration globals, four ingredient-position lists per family, option globals, Cure Disease/Cure Poison exemplars, and the two alchemy game settings. Resolution is deliberately tolerant of partial record layouts: editor IDs are tried first, then the active plugin name, both known CACO plugin names, and finally `Update.esm` for documented local FormIDs. The plugin name and local form IDs are used as fallbacks because CACO records can be exposed through different loaded-file arrangements.

`detected` means that the CACO plugin or recognizable CACO records were found. `active` requires a calculation source (the CACO plugin or core/ingredient records) and the two live Skyrim alchemy game settings; optional handling globals and individual duration lists do not block the adapter. Missing optional records disable only the dependent behavior. If the calculation source or settings are unavailable, the adapter remains inactive and the existing non-CACO path is preserved. The adapter exposes its active state and calculation revision to the prediction engine; a calculation revision changes when duration selectors or supported CACO option state changes, which invalidates the recommendation cache. The plugin does not write CACO diagnostics to a log file.

#### Mixed effects, duration records, and safe fallbacks

The evaluator groups shared effects by their source `EffectSetting`, keeps every effect shared by at least two recipe ingredients, and orders the selected effects by the CACO-native contribution estimate. It then evaluates the complete selected set rather than assuming that a recipe is purely beneficial or purely harmful. A mixed potion is identified when the selected effects contain both beneficial and harmful effects; this state is passed into the CACO perk-entry evaluation, enables the optional impure post-processing when CACO's live option permits it, and prevents the pure one-effect Crucible exemplar path from being used. The primary selected effect still determines the potion/poison classification and display control effect.

For each ingredient effect, the adapter resolves CACO's active duration variant only when the ingredient belongs to the corresponding CACO family list. It reads the family duration global and selects the 1-second, 5-second, or 10-second record. Duration-based classification uses the `MagicAlchDurationBased` keyword and the loaded magnitude/duration flags, covering CACO records that reclassify effects such as Silence, Resist Disease, or Slow as duration-based. Blood and undead variants prefer their alternate records, then the available legacy `DLC1` effect names. If a family list or selected variant is missing, the source ingredient effect is retained instead of inventing a cost or magnitude from a hardcoded table.

Numeric inputs and record lookups are validated before they reach the CACO-native estimate. Missing or invalid optional data disables the dependent compatibility operation; an invalid active native candidate is rejected rather than producing a non-finite value. Initialization and refresh exceptions also clear the optional CACO processing gates. If the adapter cannot establish a valid active calculation source, the evaluator uses the non-CACO path. These fallbacks keep partial/legacy installations from mutating menu state or silently mixing incomplete records into a prediction, but they cannot guarantee exact parity with Skyrim's private potion-construction routine.
